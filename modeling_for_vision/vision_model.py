...
from modeling_for_vision.modules.base_model import BaseModel
from modeling_for_vision.modules.blocks import VMEncoder, VMDecoder
from modeling_for_vision.quantizer.quantizer import VectorQuantizer, DiagonalGaussianDistribution
from modeling_for_vision.modules.maskgit_vqgan import Encoder as Pixel_Eecoder
from modeling_for_vision.modules.maskgit_vqgan import Decoder as Pixel_Decoder
from modeling_for_vision.modules.maskgit_vqgan import VectorQuantizer as Pixel_Quantizer
import json
...
class VisionModel(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    ...
    def decode_multi_tokens(self, tokens1, tokens2, alpha=0.5):
        if self.quantize_mode == "vq":
            ...
            z1_quantized, z2_quantized = torch.chunk(z_quantized, 2, dim=2)
            z_quantized = z2_quantized * alpha + z1_quantized * (1 - alpha)
            z_quantized = rearrange(z_quantized, 'b h w c -> b c h w').contiguous()

        elif self.quantize_mode == "vae":

            print('self.quantize_mode == "vae"')
            tokens = tokens1 * alpha + tokens2 * (1 - alpha)
            z_quantized = tokens
